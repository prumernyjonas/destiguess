-- Kompletní migrace: Přejmenování pano_url na image_url + přidání lokací
-- Spusť tento SQL v Supabase SQL Editor

-- Krok 1: Přejmenovat sloupec pano_url na image_url (pokud ještě neexistuje)
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'locations' AND column_name = 'pano_url'
  ) THEN
    ALTER TABLE locations RENAME COLUMN pano_url TO image_url;
  END IF;
END $$;

-- Krok 2: Přidat unique constraint na title (pokud ještě neexistuje)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'locations_title_unique'
  ) THEN
    ALTER TABLE locations ADD CONSTRAINT locations_title_unique UNIQUE (title);
  END IF;
END $$;

-- Krok 3: Smaž všechny existující lokace (volitelné - odkomentuj pokud chceš začít znovu)
-- DELETE FROM locations;

-- Krok 4: Přidej lokace s obrázky
